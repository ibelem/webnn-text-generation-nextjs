 ...
 */
async function generatePipeline({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, selectedModelObj, maxOutputTokens, maxInputTokens }) {
 ...
 */
async function generateVLM({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, selectedModelObj, maxOutputTokens, maxInputTokens }) {
  ...
  if (hasNewImage) {
  ...
  } else if (supportsVLMPKV && vlm_prompt_history !== null && vlm_past_key_values !== null) {
    // Continuation turn on WebGPU: reuse cached KV, text-only re-encode
    ...
    generateArgs = {
      ...
      ...(vlm_image_grid_thw ? { image_grid_thw: vlm_image_grid_thw } : {}),
    };
  } else {
  ...
  // Cache PKV and decoded prompt history for the next continuation turn
  if (supportsVLMPKV && vlmGenResult && "past_key_values" in vlmGenResult) {
...
async function generate({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, maxOutputTokens, maxInputTokens }) {
  const selectedModelObj = MODELS.find((m) => m.id === currentModelId);

  // Vision-Language Model path (conditional-generation with AutoProcessor)
  if (selectedModelObj?.modelClass === "conditional-generation") {
    return generateVLM({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, selectedModelObj, maxOutputTokens, maxInputTokens });
  }

  // GPT-OSS pipeline path (uses pipeline() + Harmony stream format)
  if (selectedModelObj?.usePipeline) {
    return generatePipeline({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, selectedModelObj, maxOutputTokens, maxInputTokens });
  }
