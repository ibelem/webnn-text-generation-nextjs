// Polyfill for web worker environment - must be set before any module that touches `window`
self.window = self;

// Static imports for local modules - webpack inlines these into the worker bundle directly.
// (These modules don't reference `window`, so the polyfill order doesn't matter for them.)
import { MODELS } from "./constants";
import { getHuggingFaceDomain, isRemoteEnvironment } from "./utils";
import { HarmonyStreamParser } from "./harmony";

// Load transformers.js from the Next.js public folder at runtime.
// /* webpackIgnore: true */ prevents webpack from treating this as a chunk —
// it's a self-contained build served as a static asset from /public.
const _transformersUrl = new URL("/transformersjs-qwen3/transformers.js", self.location.origin).href;
const { AutoTokenizer, AutoModelForCausalLM, TextStreamer, InterruptableStoppingCriteria, pipeline, env } = await import(/* webpackIgnore: true */ _transformersUrl);

const wasmPathPrefix = `https://cdn.jsdelivr.net/npm/onnxruntime-web@1.25.0-dev.20260221-b2a6e69e82/dist/`;
