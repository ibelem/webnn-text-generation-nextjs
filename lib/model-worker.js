...
const { AutoTokenizer, AutoModelForCausalLM, TextStreamer, InterruptableStoppingCriteria, env } = await import("../public/transformers_js_v4_pre_build_20260205/transformers.js");
const { MODELS } = await import("./constants");
const { getHuggingFaceDomain, isRemoteEnvironment } = await import("./utils");

const wasmPathPrefix = `https://cdn.jsdelivr.net/npm/onnxruntime-web@1.25.0-dev.20260125-727db0d3dc/dist/`;
...
async function generateGeneric({ messages, reasonEnabled, writingAssistantEnabled, writingAssistantPrompt }) { // Add writingAssistantEnabled
  ...
  if (currentModelId === "qwen3-0_6b" || currentModelId === "llama_3-2_1b") {
    genParams.past_key_values = past_key_values_cache;
  }

  const result = await model.generate(genParams);

  if ((currentModelId === "qwen3-0_6b" || currentModelId === "llama_3-2_1b") && result.past_key_values) {
