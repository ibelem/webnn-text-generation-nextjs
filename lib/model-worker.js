...
async function generate({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, maxOutputTokens, minOutputTokens, maxInputTokens, temperature }) {
  ...
  // enable_thinking is driven purely by the reasonEnabled toggle. thinkingTagSupport
  // only indicates whether the model emits <think></think> tags (used below for UI
  // state tracking) — it must not gate whether thinking can be turned on/off.
  // It must also be passed as a top-level option, not nested under
  // chat_template_kwargs: transformers.js's apply_chat_template only merges
  // unrecognized top-level keys into the Jinja render context, so a nested
  // chat_template_kwargs object is never unpacked and silently has no effect.
