...
async function generatePipeline({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, selectedModelObj, maxOutputTokens, minOutputTokens, maxInputTokens, temperature }) {
  ...
  try {
    ...
    await generator(apiMessages, {
      ...
      // min_new_tokens: use explicit minOutputTokens if set, otherwise match
      // maxOutputTokens to suppress early EOS (min == max).
      ...(minOutputTokens ? { min_new_tokens: minOutputTokens } : maxOutputTokens ? { min_new_tokens: maxOutputTokens } : {}),
      do_sample,
...
async function generate({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, maxOutputTokens, minOutputTokens, maxInputTokens, temperature }) {
  ...
  const inputs = tokenizer.apply_chat_template(patchedMessages, {
    add_generation_prompt: true,
    return_dict: true,
    ...(selectedModelObj.thinkingTagSupport && { chat_template_kwargs: { enable_thinking: reasonEnabled } }),
  });
