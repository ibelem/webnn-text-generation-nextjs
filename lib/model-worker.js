  ...
  const callback_function = (output) => {
    ...
    if (delta) {
      if (delta.type === "new_message") {
        ...
        return; // Header token — no content to emit yet
      }
      if (delta.type === "content") {
        const msg = parser.messages[delta.messageIndex];
        const channel = msg?.channel ?? "";

        // Map "analysis" channel to "thinking" state
        if (channel === "analysis") {
          state = "thinking";
        } else {
          state = "answering";
      ...
      // "done" deltas don't need to produce updates
    } else if (!harmonyDetected) {
      // Fallback: model is generating plain text without Harmony format.
      // Send raw tokens directly as content.
      self.postMessage({
        status: "update",
        output,
        tps,
    ...
    // If harmonyDetected && delta is null → it's a header/special token; skip
  };

  const streamer = new TextStreamer(tokenizer, {
    skip_prompt: true,
    skip_special_tokens: false, // Keep Harmony special tokens for parsing
    callback_function,
