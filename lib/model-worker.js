 ...
 */
async function generatePipeline({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, selectedModelObj, maxOutputTokens, maxInputTokens, temperature }) {
  ...
  try {
    ...
    await generator(apiMessages, {
      ...
      // When the user sets Max Output Tokens, force generation to that exact
      // length by suppressing early EOS (min == max).
      ...(maxOutputTokens ? { min_new_tokens: maxOutputTokens } : {}),
      do_sample,
 ...
 */
async function generateVLM({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, selectedModelObj, maxOutputTokens, maxInputTokens, temperature }) {
...
async function generate({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, maxOutputTokens, maxInputTokens, temperature }) {
  const selectedModelObj = MODELS.find((m) => m.id === currentModelId);

  // Vision-Language Model path (conditional-generation with AutoProcessor)
  if (selectedModelObj?.modelClass === "conditional-generation") {
    return generateVLM({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, audio, selectedModelObj, maxOutputTokens, maxInputTokens, temperature });
  }

  // GPT-OSS pipeline path (uses pipeline() + Harmony stream format)
  if (selectedModelObj?.usePipeline) {
    return generatePipeline({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, selectedModelObj, maxOutputTokens, maxInputTokens, temperature });
  }
