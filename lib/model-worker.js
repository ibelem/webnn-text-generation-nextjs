...
class TextGenerationPipeline {
  static async getInstance(progress_callback = null) {
    ...
    // Vision-Language Models (conditional-generation) — use AutoProcessor + AutoModelForVision2Seq
    if (selectedModelObj?.modelClass === "conditional-generation") {
      this.processor ??= AutoProcessor.from_pretrained(hfModelId, {
        progress_callback,
      });
      this.model ??= AutoModelForVision2Seq.from_pretrained(hfModelId, {
 ...
 * Uses AutoProcessor + AutoModelForVision2Seq to handle image+text inputs.
 * Images are received as data URLs or blob URLs from the main thread.
 */
async function generateVLM({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, selectedModelObj }) {
  ...
  contentParts.push({ type: "text", text: textContent || "Describe what you see." });
  ...
  const text = processor.apply_chat_template(chatMessages, {
    ...
    tokenizer_kwargs: { enable_thinking: reasonEnabled },
  });
  ...
  const inputs = await processor(text, rawImages.length > 0 ? rawImages : null);
...
async function generate({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images }) {
  const selectedModelObj = MODELS.find((m) => m.id === currentModelId);

  // Vision-Language Model path (conditional-generation with AutoProcessor)
  if (selectedModelObj?.modelClass === "conditional-generation") {
    return generateVLM({ messages, reasonEnabled, systemPromptEnabled, systemPromptText, images, selectedModelObj });
  }
...
async function load() {
  try {
    ...
    if (selectedModelObj?.modelClass === "conditional-generation") {
      ...
      const warmupInputs = await processor(warmupText);
