...
const MODEL_ID_MAP = {
  ...
  "smollm3-3b": "HuggingFaceTB/SmolLM3-3B-ONNX", // <-- Add this line
};
...
async function generateSmolLM3({ messages, reasonEnabled }) {
  const [tokenizer, model] = await TextGenerationPipeline.getInstance();

  // Insert /think or /no_think in system prompt if needed
  ...
  if (systemMessageIdx !== -1) {
    ...
    patchedMessages = [
      { role: "system", content: `You are SmolLM, a language model created by Hugging Face. ${thinkingTag}` },
      ...patchedMessages,
    ];
  }
