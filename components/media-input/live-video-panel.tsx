...
import { Play, Square, Video, Upload, Loader2, Menu, X, Sparkles } from "lucide-react"
...
export function LiveVideoPanel({
  ...
  const initCamera = useCallback(async () => {
    try {
      ...
      setResponse(`Error accessing camera: ${error.name} - ${error.message}`);
      return false;
    }
  ...
  useEffect(() => {
    ...
    const onMessage = (e: MessageEvent) => {
      const { status, output, tps, numTokens } = e.data;
      if (status === "update") {
  ...
  // Initialize camera on mount if no source
  useEffect(() => {
    if (anyModelReady && !stream && !fileObjectUrl) {
      initCamera();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [anyModelReady]);
  ...
  const handleStart = () => {
    if (!anyModelReady) {
      setResponse("Model not ready. Please load a model first.");
      return;
    }

    const video = videoRef.current;
    if (!video || video.videoWidth === 0) {
      setResponse("Video source not ready.");
      return;
    }
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-6 flex flex-col items-center gap-4 bg-gradient-to-b from-gray-50 to-white">
        {/* Video container */}
        <div className="relative w-full max-w-[640px] aspect-video bg-black rounded-xl overflow-hidden border border-gray-200 shadow-lg">
          <video
            ref={videoRef}
            autoPlay
            playsInline
          ...
          <button
            ...
            className="absolute top-2.5 left-2.5 z-10 px-2.5 py-1.5 text-[11px] font-semibold text-white bg-black/60 hover:bg-black/75 rounded-full border border-white/20 backdrop-blur-sm transition-all opacity-0 hover:opacity-100 focus:opacity-100 disabled:opacity-30 disabled:cursor-not-allowed group-hover:opacity-100"
            style={{ opacity: isProcessing ? 0.3 : undefined }}
            onMouseEnter={(e) => {
              if (!isProcessing) (e.currentTarget.style.opacity = "1");
            }}
            onMouseLeave={(e) => {
              if (!isProcessing) (e.currentTarget.style.opacity = "0");
            }}
          >
            {sourceMode === "webcam" ? (
            ...
            <div className="flex-shrink-0">
              {isProcessing ? (
                ...
                <Button
                  ...
                  className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold px-5 py-2 rounded-lg disabled:opacity-50 hover:cursor-pointer"
                >
